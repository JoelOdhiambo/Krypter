...
from tkinter import DISABLED, Frame, Label, PhotoImage, StringVar, ttk
...
class KrypterApp(tk.Tk, ciphers.twofish_cipher.TwofishCipher, ciphers.aes_cipher.AesCipher):
    def __init__(self):
        ...
        self.original_file_entry = ttk.Entry(self.encrypt_frame, width=44, state='disabled')
        ...
        self.password_entry_one.grid(column=0, row=3, sticky=tk.W, padx=20, pady=5)
        ...
        # state of radiobuttons '0' for None are active_radio_button '1' for AES Encryption '2' for Twofish Encryption '3' for AES
        # Decryption '4' for Twofish Decryption.
        ...
        self.password_entry_two = ttk.Entry(
            self.decrypt_frame, width=44)
        self.password_entry_two.grid(column=0, row=3, sticky=tk.W, padx=20, pady=5)
        ...
        self.progress_bar = ttk.Progressbar(self.status_bar, orient='horizontal', mode='indeterminate', length=100)
    ...
    def radio_button_state(self):
        if self.aes_radio_button_e or self.two_fish_radio_button_e:
            self.encrypt_button.config(state='enabled')

        if self.aes_radio_button_d or self.two_fish_radio_button_d:
    ...
    def active_radio_button(self):
        ...
        if self.radio_var.get() == '1':
            ...
            aes_cipher(self.original_file, self.password_entry_one.get()).encrypt_file()
        ...
        if self.radio_var.get() == '2':
            ...
            two_fish(self.original_file, self.password_entry_one.get()).encrypt_file()
            self.encrypted_file_dest.config(textvariable=self.encrypted_file_dir_var)
        ...
        if self.radio_var_two.get() == '3':
            ...
            aes_cipher(self.encrypted_file, self.password_entry_two.get()).decrypt_file()
        ...
        if self.radio_var_two.get() == '4':
            ...
            two_fish(self.encrypted_file, self.password_entry_two.get()).decrypt_file()
            self.decrypted_file_path.config(textvariable=self.decrypted_file_dir_var)
